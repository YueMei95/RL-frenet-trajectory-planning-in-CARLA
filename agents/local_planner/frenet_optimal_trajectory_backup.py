...
class MotionPlanner:
    ...
    def run_step(self, c_speed):
        ...
        np.hypot(path.x[1] - tx[-1], path.y[1] - ty[-1]) <= 1.0:
        """
        t0 = time.time()
        best_path_idx, fplist = frenet_optimal_planning(self.csp, self.s0, c_speed, self.c_d, self.c_d_d, self.c_d_dd, self.ob)
        print('trajectory planning time: {} s'.format(time.time() - t0))
        self.s0 = fplist[best_path_idx].s[-1]
        self.c_d = fplist[best_path_idx].d[-1]
        self.c_d_d = fplist[best_path_idx].d_d[-1]
        self.c_d_dd = fplist[best_path_idx].d_dd[-1]
        # self.c_speed = fplist[bestpath_idx].s_d[1]
        return best_path_idx, fplist
