...
class CarlaGymEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # RL
        self.low_state = np.array([[-1 for _ in range(self.look_back)] for _ in range(2)])
        self.high_state = np.array([[1 for _ in range(self.look_back)] for _ in range(2)])

        # self.observation_space = gym.spaces.Box(low=-self.low_state, high=self.high_state,
        #                                         dtype=np.float32)

        self.observation_space = gym.spaces.Box(low=-1, high=1, shape=(self.look_back, 2),
                                                dtype=np.float32)
    ...
    def step(self, action=None):
        ...
        lstm_obs = np.concatenate((np.array(speeds_vec)[-self.look_back:], np.array(ego_norm_d)[-self.look_back:]), axis=0)
