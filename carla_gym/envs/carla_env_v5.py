...
class CarlaGymEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # RL
        self.low_state = np.array([[-1 for _ in range(self.look_back)] for _ in range(3)])
        self.high_state = np.array([[1 for _ in range(self.look_back)] for _ in range(3)])

        # self.observation_space = gym.spaces.Box(low=-self.low_state, high=self.high_state,
        #                                         dtype=np.float32)

        self.observation_space = gym.spaces.Box(low=-1, high=1, shape=(self.look_back, 3),
                                                dtype=np.float32)
    ...
    def step(self, action=None):
        ...
        while self.f_idx < wps_to_go and elapsed_time(path_start_time) < self.motionPlanner.D_T * 1.5:
            ...
            ego_norm_s.append((ego_s-self.init_s) / self.max_s)
        ...
        speeds_vec = (np.array(speeds)-self.targetSpeed)/self.maxSpeed
        lstm_obs = np.concatenate((np.array(speeds_vec)[-self.look_back:], np.array(ego_norm_s)[-self.look_back:], np.array(ego_norm_d)[-self.look_back:]), axis=0)
