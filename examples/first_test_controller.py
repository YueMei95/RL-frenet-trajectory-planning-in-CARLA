...
from agents.strl.basic_agent import BasicAgent
...
class World(object):
    ...
    def restart(self):
        ...
        if self.player is not None:
            self.destroy()
        transform = carla.Transform(carla.Location(x=402.242, y=-97.558, z=1.20001),
...
def game_loop(args):
    ...
    try:
        ...
        # world.Config(synchronous=True, no_rendering=False, time_step=0.05)
        dt = 1.0 / 20.0
        args_lateral_dict = {
            'K_P': 1.95,
            'K_D': 0.01,
        ...
        args_longitudinal_dict = {
            ...
            'K_I': 1,
            'dt': dt}
        vehicleController = VehiclePIDController(world.player,
        ...
        while True:
            ...
            clock.tick()
            world.tick(clock)
            ts = world.world.wait_for_tick()
            world.render(display)
            pygame.display.flip()

            # Set control and apply
            tr = world.player.get_transform()
            print(tr)
            tr.location.y -= 3
            tr.location.x += 1
            world.player.set_transform(tr)
            # wp = carla.Transform(carla.Location(x=402.302, y=-250, z=0), carla.Rotation(pitch=360, yaw=270.599, roll=0))
            # print(wp)
            control = vehicleController.run_step(100, 0)
